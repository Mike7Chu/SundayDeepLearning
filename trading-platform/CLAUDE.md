...
- ✅ **발굴 레이더(급등 전조 스크리너)**(`api/services/stock_radar.py`, `GET /stocks/radar`): 전 시장에서 '터질 종목'의
  ...
  **종목별 수급(외인·기관 매집)**: 토스 `/market-indicators/{symbol}/investor-trading`로
  상위 후보 5일 순매수 조회 → `supply_demand`(외인+기관 합, +100억=만점 15점 보너스·
  매집/분산 사유). 한국장 급등의 핵심 선행지표. 상세 모달 주가흐름 섹션에도 표시(10분 캐시).
...
- ✅ **포트폴리오 리스크(사실상 1베팅 경고)**: `api/services/portfolio_risk.py`(순수함수)·`GET /portfolio/risk`.
...
- ✅ **전략 리팩터(리포트 기반)**: 28건 실측 성적 진단(net이 gross의 12%뿐 — 비용이 88% 잠식,
  ...
  단타 신규진입·스윙 매수 3지점) · 확신 하한(`plan_min_swing=50`)으로 약한 후보는 매수 대신 현금.
  ②**시황 라우터**(`engine/regime.py` 순수): 코스피 200일선·변동성·외국인 수급으로 4국면
  (강세추세/중립/횡보/위험회피) 판정 → 국면별 활성 전략 ID(S1~S5). `_update_regime`이 10분마다
  `ENGINE_REGIME_KEY` 갱신, 에이전트 컨텍스트·브리핑·플랜 카드에 노출. ③**전략 5**(`engine/strategies.py`
  순수): S1 추세모멘텀·S2 퀄리티밸류(TTM)·S3 저변동방어·S4 단기평균회귀·S5 촉매수급. 공통 서명
  (q,candles)→픽|None(재무 결측이면 None=억지매수 금지). `run_strategies`가 국면 활성 전략 중
  최고 점수 픽 채택 → `_swing_plan`의 매수 선택을 구동. 검증: 가격기반(S1·S4) 백테스트, 교차섹션
  (S2·S3·S5) 포워드로그. 예측 아님·최종 심판은 비용 반영 net. 리포트 아티팩트로 전체 설계 문서화.

...
# 프로세스 전역 스로틀(여러 TossClient·여러 수집 루프가 공유) — 토스 레이트리밋 방어.
_throttle_lock = asyncio.Lock()
_last_call_ts = 0.0


async def _throttle() -> None:
    """토스 요청 사이 최소 간격 보장(전 루프 직렬화) — 버스트로 한도 초과 방지."""
    global _last_call_ts
    async with _throttle_lock:
        wait = settings.toss_min_interval_sec - (time.monotonic() - _last_call_ts)
        if wait > 0:
...
class TossClient:
    ...
    async def _request(self, method: str, client: httpx.AsyncClient, path: str, *,
                       params: dict | None = None, json: dict | None = None,
                       account: str | None = None):
        """스로틀 + 재시도 공통 경로 — rate-limit-exceeded는 백오프 재시도,
        invalid-token은 캐시 토큰 폐기 후 재발급 재시도(전 읽기·주문 공유)."""
        last: Exception | None = None
        for attempt in range(max(1, settings.toss_max_retry)):
            await _throttle()
            ...
            try:
                ...
                last = exc
                if attempt >= settings.toss_max_retry - 1:
                    raise
                if exc.code == "rate-limit-exceeded":
                    await asyncio.sleep(0.5 * (2 ** attempt))   # 0.5·1·2s 백오프
                    continue
                if exc.code in ("invalid-token", "unauthorized", "token-expired"):
    ...
    async def _get(self, client: httpx.AsyncClient, path: str, *,
        ...
        return await self._request("GET", client, path, params=params,
                                   account=account)

    async def _post(self, client: httpx.AsyncClient, path: str, *,
                    json: dict | None = None, account: str | None = None):
        return await self._request("POST", client, path, json=json,
                                   account=account)

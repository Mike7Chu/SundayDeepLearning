...
class Analyst:
    ...
    async def analyze_coach(self, block: str) -> dict:
        ...
        prompt = (
            ...
            "경고 문구를 쓰지 마세요.\n"
            "'[미국 반도체]'·'[AI 인프라 투자(CAPEX) 프록시]' 블록이 제공되면 **그 수치를 "
            "근거로** 미국 동향 섹션을 작성하세요(우리 시스템이 토스에서 수집한 실제 전일 "
            "종가). 웹 검색이 가능하면 관련 뉴스로 보강해도 좋지만 필수는 아닙니다. "
            "블록이 없고 검색도 불가할 때만 '확인 불가'로 표시하세요(추측 금지).\n"
            "[의견 규율 — 단정 금지]\n"

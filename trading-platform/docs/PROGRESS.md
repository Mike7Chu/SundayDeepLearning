...
## 현재 한 줄 상태
Phase 0~1 완료 + Phase 2 코어(김프 계산/REST/WS) 완료. 테스트 3/3 통과. RPi 배포 스크립트 준비됨. **대시보드 UI·알림봇·펀비·봇·주식은 미착수.**
...
## 다음 작업 (우선순위)
1. **텔레그램 알림봇** (`notifier/`) — 김프/역프/펀비 임계치 → 텔레그램. *시스템 핵심.*
2. **공지알림봇** — 업비트/빗썸 공지 폴링(`collector/announcements/`) → 신규상장 알림.
3. **펀비 수집 + 펀비 대시보드** — 무기한선물 펀딩비 추가.
